...
LOG_DIR="/home/mcsim/logs/synchronizer"
STRATEGY_DIR="/home/mcsim/IWE/DS-strategy"

# Agent Workspace: если существует — отчёты идут туда
AGENT_WORKSPACE="/home/mcsim/IWE/DS-agent-workspace"
...
STRATEGIST_LOG="/home/mcsim/logs/strategist/$DATE.log"
...
"
        if echo "$warnings" | grep -q "push failed" 2>/dev/null; then
            report+="- **push failed:** Mac был оффлайн. Запусти \`cd /home/mcsim/IWE/DS-strategy && git pull --rebase && git push\`
"
        fi
    else
        report+="
